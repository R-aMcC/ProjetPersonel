...
def isflt(i, nomvar):
    #Vérifie qu'un variable est un float
    try:
        float(i)
        return (True)
    except:
        print()
        print(f'Ce variable doit être un nombre (reçu "{i}")')
        print()
        return (False)
...
def quad_eq(a, b, c):
    ...
    #Valeurs qui fonctionnent bien: a=1, b=5, c=6
    #RÉSULTATS: -3, -2
    if a == 0:
        ...
        if isinstance(x1, complex):
            ...
            print()
            print(
                "Il n'y a pas de vraie réponses à cette équation."
            )
        else:
            (round(x1, 4))
...
def cubic_eq(a, b, c, d):
    ...
    #Valeurs qui fonctionnent bien: a=1 b=3 c=-10 d=0 
    # RÉSULTAT: -5, 0, 2
...
def quart_eq(a, b, c, d, e):
    #Ce fonction calcule la les 4 racines, ou réponses, de l'équation.
     #Valeurs qui fonctionnent bien: a=1 b=6 c=3 d=-10 e=0
    # RÉSULTAT: -5, -2, 0, 1
    if a == 0:
        print(
            "Le variable a ne peut pas être 0, sinon l'équation n'est pas quartique"
        )
    else:
        #Simplifiant les variables pour les faires plus facile en divisant chaque variable par la valeur de a
...
def func():
    #Créé une boucle infini, qui fait toute fonctionner. Elle prend les informations données par l'utilisateur et décide quoi faire.
    loop = 1
    ...
    while loop == 1:
        print(' ')
        inp = input(
            "SVP entrer le numéro qui correspond à la fonction dont tu veux calculer :  "
        )
        print()
        ...
        if inp == '1':
            ...
            a = getflt('touver la racine de la prochaine variable ')
            b = getflt('la dégrée de la racine (Positif)')
            if b < 0:
                print()
                print('SVP enter un nombre positif  pour b')
            # Asure que les données fonctionnent
            elif (b % 2) == 0:
                if (a < 0):
                    print()
            ...
            print()
            print(
                "Cette fonction prends 3 variables, a, b, et c, et trouve les 2 racines, ou réponses, (Valeur de x) de l'équation. Elle est un polynome du 2e degrée."
            )
            print()
            print()
            print(
                '2 = Équation cubique de la forme "ax^3 + bx^2 + cx + d = 0"')
            print()
            print(
                "Cette fonction prend 4 variables, a, b, c et d, et trouve les 3 racines, ou réponses, (Valeur de x) de l'équation. Les 3 racines peuvent être soit : Tous les 3 égales et réelles, les 3 sont réelles,  et une réelle et 2 complexes (de la forme n+m*1 où m et n sont des nombres décimales). Elle est un polynôme du 3e degrée"
            )
            print()
            print()
            print(
                '3 = Équation quarique de la forme "ax^4 + bx^3 + cx^2 + dx + e = 0"'
            )
            print()
            print()
            print(
                "Cette fonction prends 5 variables, a, b, c, d et c, et trouve les 4 racines, ou réponses, (Valeurs de x ou y =0) de l'équation. Parfois, certaines des racines sont complexe, alors sous la forme de (n + m*i) où n est un nombre décimale et m est un nombre décimale.Cette fonction est la plus grade fonction qui est possible d'être résoud avec un radical, et est un polynome du 4e degrée."
            )
        else:
